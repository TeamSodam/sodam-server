...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (existUser.length !== 0) {
      return res.status(statusCode.BAD_REQUEST).send(util.fail(statusCode.BAD_REQUEST, responseMessage.NICKNAME_EXIST));
    }

    // 같은 닉네임 없는 경우 (수정 가능)
    await userDB.postUserNicknameByUserId(client, userId, nickname);
    const result = {
      nickname,
    };
    return res.status(statusCode.OK).send(util.success(statusCode.OK, responseMessage.UPDATE_ONE_USER_SUCCESS, result));
  } catch (error) {
